...
import {
  Bot, RefreshCw, CheckCircle, XCircle, Clock, Minus,
  Trash2, TrendingUp, BarChart2, Cpu, ChevronDown, ChevronUp,
} from "lucide-react";
...
const ENGINE_CATEGORY_COLORS: Record<string, string> = {
  markov1: "text-violet-400 bg-violet-500/10 border-violet-500/20",
  ...
  adaptive: "text-primary bg-primary/10 border-primary/20",
};
...
function getConfidence(bd: EngineBreakdown): number | null {
  return typeof bd.confidence === "number" ? bd.confidence : null;
}
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-1.5">
      {engines.map((e) => (
        <div key={e.id} className={cn(
          ...
          e.signal ? ENGINE_CATEGORY_COLORS[e.id] ?? "text-foreground bg-secondary border-border"
          ...
          <div className="flex items-center gap-1.5 min-w-0">
            ...
            <span className="truncate">{V5_ENGINE_LABELS[e.id] ?? e.label}</span>
          </div>
    ...
    <div className="bg-card border border-border rounded-sm overflow-hidden">
      ...
      >
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-2">
            ...
            {signalCount > 0 && (
              <span className="text-[10px] font-mono text-violet-400/70">{signalCount}/{activeCount} engines aktif</span>
            )}
          </div>
          <div className="flex items-center gap-3 flex-wrap">
            <span className="font-mono text-2xl font-bold text-violet-400 tracking-widest">{row.prediction}</span>
            {row.actualResult && (
              <>
      ...
      <div className="flex items-center gap-4 px-4 pb-3 flex-wrap">
        ...
        {conf !== null && (
          <div>
            ...
            <div className={cn("font-mono text-sm font-bold",
              conf >= 0.70 ? "text-emerald-400" : conf >= 0.55 ? "text-amber-400" : "text-red-400"
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          >
            <Cpu className="w-3 h-3" /> Detail Engine
          </button>
...
function EngineAccuracyTable({ records }: { records: Parsed[] }) {
  ...
  for (const row of resolved) {
    const engines = row.bd.engines ?? [];
    for (const e of engines) {
      if (!stats[e.id]) {
        stats[e.id] = { id: e.id, label: V5_ENGINE_LABELS[e.id] ?? e.label, appearances: 0, signal: 0, wins: 0, partial: 0, totalPct: 0 };
      }
  ...
  const sorted = Object.values(stats)
    .filter((s) => s.signal > 0)
    .sort((a, b) => b.wins - a.wins || b.signal - a.signal);
                ...
                <tr key={s.id} className="border-b border-border/40 hover:bg-secondary/20 transition-colors">
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-1.5">
                      <div className={cn("w-1.5 h-1.5 rounded-full", ENGINE_CATEGORY_COLORS[s.id]?.includes("violet") ? "bg-violet-400" : ENGINE_CATEGORY_COLORS[s.id]?.includes("blue") ? "bg-blue-400" : ENGINE_CATEGORY_COLORS[s.id]?.includes("amber") ? "bg-amber-400" : ENGINE_CATEGORY_COLORS[s.id]?.includes("cyan") ? "bg-cyan-400" : ENGINE_CATEGORY_COLORS[s.id]?.includes("pink") ? "bg-pink-400" : ENGINE_CATEGORY_COLORS[s.id]?.includes("green") ? "bg-green-400" : "bg-primary")} />
                  ...
                  <td className="px-4 py-2">
                    <span className="text-amber-400">{s.partial}</span>
                  </td>
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className={cn("font-bold", winRate >= 0.3 ? "text-emerald-400" : winRate >= 0.1 ? "text-amber-400" : "text-red-400")}>
...
// ─── Accuracy mini-chart (last 30 resolved) ───────────────────────────────────
    ...
    <div className="bg-card border border-border rounded-sm p-4">
      ...
      <div className="flex items-end gap-1 h-16">
        {resolved.map((r, i) => {
          ...
          const heightPct = (matched / max) * 100;
          return (
            <div key={r.id} className="flex-1 flex flex-col items-center justify-end" title={`${r.date} ${r.period}: ${matched}/4`}>
              <div
                className={cn("w-full rounded-sm transition-all", matched === 4 ? "bg-emerald-400" : matched >= 2 ? "bg-amber-400" : matched === 1 ? "bg-orange-400" : "bg-red-400/60")}
...
export default function PredictionHistoryV5() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/prediction/v5/history?${params}`);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const raw = await res.json() as HistoryRecord[];
      setData(raw.map((r) => ({ ...r, bd: parseBd(r.engineBreakdown) })));
    } catch (e) {
      setError(e instanceof Error ? e.message : "Gagal memuat riwayat V5");
    } finally {
      setLoading(false);
    }
  }, [period, limit]);
    ...
    <div className="space-y-5 pb-20">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 text-violet-400 font-mono text-xs uppercase tracking-widest mb-1">
            <Bot className="w-4 h-4" /> Riwayat Prediksi V5
          </div>
          <h1 className="text-2xl font-bold font-mono">Riwayat Prediksi V5</h1>
          <p className="text-muted-foreground text-sm mt-1 font-mono">
            Rekam jejak &amp; akurasi 13 engine V5 — Autonomous Data Scientist.
          </p>
        </div>
        <button
      ...
      {data && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          {[
            ...
            { label: "Rata-rata", value: resolved.length ? `${(avgAccuracy * 100).toFixed(1)}%` : "—", color: "text-violet-400" },
      ...
      {resolved.length > 0 && (
        <div className="bg-card border border-border rounded-sm p-4">
          ...
          <div className="flex h-4 rounded-sm overflow-hidden gap-0.5">
            {exact > 0 && (
              <div className="bg-emerald-400 transition-all" style={{ width: `${(exact / resolved.length) * 100}%` }}
                title={`Tepat: ${exact} (${((exact / resolved.length) * 100).toFixed(0)}%)`} />
            )}
            {partial > 0 && (
              <div className="bg-amber-400 transition-all" style={{ width: `${(partial / resolved.length) * 100}%` }}
                title={`Sebagian: ${partial} (${((partial / resolved.length) * 100).toFixed(0)}%)`} />
            )}
            {miss > 0 && (
              <div className="bg-red-400/70 transition-all" style={{ width: `${(miss / resolved.length) * 100}%` }}
                title={`Meleset: ${miss} (${((miss / resolved.length) * 100).toFixed(0)}%)`} />
            )}
          </div>
      ...
      {loading && !data && (
        <div className="flex items-center justify-center py-16 text-muted-foreground font-mono text-sm">
          <RefreshCw className="w-4 h-4 animate-spin mr-2" /> Memuat riwayat V5…
        </div>
      )}
          ...
          {data.length === 0 && (
            <div className="bg-card border border-border rounded-sm p-12 text-center">
              ...
              <div className="text-muted-foreground/50 font-mono text-xs mt-1">
                Buat prediksi pertama di halaman Prediksi V5.
              </div>
            </div>
          )}
